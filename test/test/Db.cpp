...
struct DBFHDR	{
	...
	short	unused_pg_siz;
	long	unused_create_dttm, unused_upd_dttm;
	long	dum1, dum2;
	};
#pragma pack(pop)
...
struct BTR	{
	...
	long	keyct;						// number of keys in this btree
...
struct BNODE	{
	...
	char	fill1[6];
	short	bnkey_ct;
	long	rh_lf_sib,rh_rt_sib;
	char	fill2[14];
	char	keys[sizeof(long)];
	};
...
#ifdef ZZZ_maybe_global_later
void	dbflush(HDL);
HDL		dlrnxt(HDL,HDL);
HDL		dlrlink(HDL,HDL);
HDL		dlrsnap(HDL,HDL);
HDL		dlrtohd (HDL,HDL);
void	*memtakelist(short,void*,void*);
void	dbcloseall(void);
#endif ZZZ_maybe_global_later
...
struct PGMAPS {
   RHDL  rh_pgmap;
   long  tot_free;		// SJH 04/05/04 - This needs checking out - it's not being properly maintained
   };
...
{
char   *pnxt;
while (plist) {pnxt=*((char **)((char*)plist)); memtake(plist); plist=pnxt;}
}
#endif
...
if	(phd)
  	{
  	if (phd==pelm && (phd=pelm->nxt)==pelm) phd=0; /* PELM only elm in ring */
  	if (pelm)
      {
...
if	(phd)
  	{
  	if (pelm) {if ((nxt=pelm->nxt)==phd) nxt=0;}
	else nxt=phd;			/* pelm==NULLHDL, first request for next */
   }
else nxt=0; /* end of ring, ring is empty */
return(nxt);
}
#endif
...
for (_bf_ct=0;_bf_ct<nbufs;_bf_ct++)
   {
	if (_bf_ct) buf->prv=&_bf_addr[_bf_ct-1];
	if (_bf_ct<nbufs-1) buf->nxt=&_bf_addr[_bf_ct+1];
    buf->pg_in_buf=EMPTY;
...
if (buf->bflags & DIRTY_PG) 
	...
	if (buf->pg_in_buf != EMPTY)	// needs flushing
		{
		flputat(buf->addr, PGSIZ, (long)(buf->pg_in_buf)*PGSIZ, buf->h_fl);
		}
...
do	{
	if (buf->h_fl==_dbfil && buf->pg_in_buf==pg) {got=YES; break;}
	if (!(buf->bflags&LOCKED)) avail=buf;
...
if (!got && read)					// this can only happen on what was a dbppgtomem() call
	...
	if (pg >= _mapmap->pgs_in_db)
		throw SE_BADPGID;
	flgetat(buf->addr,PGSIZ,(long)(buf->pg_in_buf=(ushort)pg)*PGSIZ,buf->h_fl=_dbfil);
	buf->bflags&=~(HAS_NEWPG|DIRTY_PG);  // Reset flags
	}
...
if (cell_siz < MINRECSIZ) cell_siz=MINRECSIZ;
else if (cell_siz > MAXRECSIZ) cell_siz=MAXRECSIZ;
...
if (cell_id<=0 || cell_id>((CELLTBL*)pv->celltbl)->vector_siz
|| (ofst=((char*)pv->pcell-pg))<0 || ofst>PGSIZ) 
	throw SE_BADCELLID;
}
// pg pve vector
...
PGVARS  pv;
dbpsetvars_r(&pv,rh_rec);
if (pv.rh_far_cell) return(_recadr(pv.rh_far_cell, psiz, flg));
if (flg) *flg=&pv.buf->bflags;
if (psiz) *psiz=pv.data_siz;		/* retrieve cell size less stored len*/
...
static void  bhdrupd(RHDL rhdl, RHDL root, long key_ct)
{
short *flg;
...
int cp_bytes(const void *a, const void *b) {return(memcmp(a,b,_btr_ks0));}

int cp_strn_ks0(const void *a, const void *b) {return(strancmp((const char *)a,(const char *)b,_btr_ks0));}
...
{
if (ky) memmove(ky,_h_btr->actv_key,_btr_ks0);
if (rhdl) *rhdl=*(RHDL*)(_h_btr->actv_key+_btr_ks0);
}
...
BTR	*b;
dbactv(h_db);
if ((bhdr=(BHDR*)_recadr00(rh_bhdr))==0) return(0);
...
switch (b->keytyp=bhdr->keytyp)
	{
	case DT_BYTES:	b->cmppart=cp_bytes;	break;
	case DT_STR:	b->cmppart=cp_strn_ks0;	break;
	case DT_SHORT:	b->cmppart=cp_short;	break;
	case DT_USHORT:	b->cmppart=cp_ushort;	break;
	case DT_LONG:	b->cmppart=cp_long;		break;
	case DT_ULONG:	b->cmppart=cp_ulong;	break;
	}
...
BHDR *b;
btrbind(h_btr);
if ((b=(BHDR*)_recadr1(_h_btr->rh_bhdr,&flg))==0) return(0);
if (cargo)
	{
...
{
HDL h_btr;
while((h_btr=_H_DB->btrs)!=0) btrclose(h_btr);
}
...
{
pm->lst_pg=-1;
for (int i=DBMPGSPMAP;i--;) pm->map_free_sp[i]=PGSIZ-FOOTERSIZ;
pm->rh_nxt_pgmap=0;
}
...
{
Xecho("freespace:%d\n",Bugbug);
if (!zz) zz=&_mapmap->pgmaps[0].tot_free;
...
while (scn < prv_sz)					/* this loop compresses the page */
   if ((cur_siz=*((short *)(&pg[scn])))&CELL_CONTAINS_DATA)
      ...
      if (scn!=fre) memmove(&pg[fre],&pg[scn], cell_siz);
...
{
short pm_num;
for (pm_num=0;pgnum>=DBMPGSPMAP;pm_num++) pgnum-=DBMPGSPMAP;
*ppg_in_map=pgnum;
return(pm_num);
}
...
short   prv;
dbpsetvars_r(&pv,rh_rec);
if (pv.rh_far_cell) return(dbplockpage(pv.rh_far_cell));
#pragma warning(push,3)
...
PGVARS  pv;
dbpsetvars_r(&pv,rh_rec);
if (pv.rh_far_cell) dbpfreepage(pv.rh_far_cell, lck);
else SETLOCK(pv.buf->bflags,lck);
}
...
{
RHDL rh_rec;
if (siz > MAXRECSIZ) siz=MAXRECSIZ; else if (siz<0) siz=0;
...
if (pv.rh_far_cell) _recdel(pv.rh_far_cell);	// if a far cell, destroy it too
}
...
#pragma warning(pop)
pv.buf->bflags|=LOCKED;
if (pv.rh_far_cell) _recdel(pv.rh_far_cell);	// (delete far cell if used)
...
if (rhdl)
	{
	if (siz<0) siz=0; else if (siz>MAXRECSIZ) siz=MAXRECSIZ;
...
if (siz>maxsiz) siz=(short)maxsiz;
if (siz<=0) siz=0; else memmove(data,p,siz);
return(siz);
}
...
if (_h_btr->keytyp==DT_USR)
	...
	while (i-- && (ia || ib))
		{if (a[i]!=0xFF) ia=0; if (b[i]!=0xFF) ib=0;}
	if (ia || ib)
		return(ia-ib);
	}
...
delta=idx=DIV2(n);
do	{
	if (idx==n) {rc=GT;break;}
	if ((rc=cmpdocmp(elm+idx*_btr_ksz4,ky))==EQ) break;
	if (rc<0)
		idx += DIV2(delta+1);
	else
		idx -= DIV2(delta+1);
	if (!delta) break;   /* going to check same elm as last time thru */
	delta=DIV2(delta);
	} while (true);
if (rc<0 && idx<n) idx++;
#pragma warning(pop)
...
{
int	mv;
if ((mv=b->bnkey_ct-from+1)>0) bndmv(b,mv,from-delct,from);
if (delct <= b->bnkey_ct)
#pragma warning(push,3)
	b->bnkey_ct -= delct;
#pragma warning(pop)
...
{
do	{	// if n/f, & > highest in node, & this isn't rightmost (highest) node
...
if (kpos>=b->bnkey_ct && !b->rh_rt_sib)
	{kpos=b->bnkey_ct; *_btr_flg|=BK_PRCD; got=NO;}
...
if (--dpth <= 0)		// If at root, root only has one child and the root
   ...
   if (nkeys==1 && rh_descnd)
	   {btrupdrt(BNDRHDL(b,1));_recdel(rhdl);}	// Release old root
   return(NO);
   }
int		f_uflow=(b->bnkey_ct < _btr_kpn/2);
if (kpos>nkeys) fixup_parent(BTRSTKLVL(dpth),rhdl,BNDKEY(b,kpos-1));
return(f_uflow);
}
...
if (BNDRHDL(b,1)==look) {*kpos=1; *got=BNDRHDL(b,2); return(NO);} // No leaf
for (i=1; i<b->bnkey_ct; i++)		// find the leaf
   if (BNDRHDL(b,i+1)==look) {*kpos=i; *got=BNDRHDL(b,i); return(YES);}
throw SE_BADDELKEY;
}
...
fpr=*flgr;*flgr|=LOCKED;		// don't let rt swap out while getting lf!
...
#pragma warning(pop)
   ...
   if (_delkeyinnode(dpth,kpos,lf_sib)) bnduflow(dpth);
   }
else
   {
	bndbalance(lf,rt);				// Spread the keys equally between siblings
	set_dirty(flgr);
	fpl=*flgl;*flgl|=LOCKED;		// Don't let lf get swapped by parent...
...
if (_delkeyinnode(_h_btr->depth,bndrepos(),0))
   {_h_btr->actv_leaf=bnduflow(_h_btr->depth); bndrepos();}
...
{
if (ky) memmove(_h_btr->actv_key,ky,_btr_ks0);
return(travsbranch());
}
...
if ((rh_bnode=BTRPOPNODE)==0) rh_bnode=bld_new_root(); /* at root */
rh_bnode=bndscan(key,rh_bnode,&got,&insrt_pos, &b);
if (got)     /* if found, then insert child */
   {_recadr1d(rh_bnode); BNDRHDL(b,insrt_pos)=rh_child;}
else
   ...
   else add_bkey_to_node(b,rh_bnode,insrt_pos,key,rh_child);
   }
}
...
int bkydel(HDL h_btr,RHDL *rhdl,void *key)
{
btrbind(h_btr);
if (bkyfnd(key))
	{btrakeyret(rhdl,key);_bkydel();return(YES);}
return(NO);
}

void bkydel_rec(HDL btr, void *key)
{
RHDL rh;
if (bkydel(btr,&rh,key) && dbisrhdl(_h_db,rh)) recdel(_h_db,rh);
}
...
if (kpos>=ct && !b->rh_rt_sib) {*_btr_flg&=~BK_EXST; kpos=ct; ok=NO;}
bkykeyset(kpos);
return(ok);
}
...
btrbind(btr);
if (again)
	if (!*again) {bkysetpos(NO);*again=YES;}
...
btrbind(h_btr);
ok=bkyfnd(key);
if (ok && ABS(mode)<2) btrakeyret(rhdl,key);
switch (mode)
   {
   case BK_LT:	*_btr_flg|=BK_PRCD; ok=bkyprvkey(rhdl,key);
				break;
   case BK_LE:	if (!ok) ok=bkyprvkey(rhdl,key);
				break;
   case BK_GE:	if (!ok) ok=bkynxtkey(rhdl,key);
				break;
   case BK_GT:	if (ok) *_btr_flg|=BK_PRCD;
				ok=bkynxtkey(rhdl,key);
				break;
   }
if (ok) *_btr_flg&=~BK_PRCD;
return(ok);
}
...
for (n=MAXPGMAPS;n--;)					// 29/4/03 I think this 'pre-initialisation' is redundant
	pms[n].tot_free=((long)(DBMPGSPMAP)) * MAXRECSIZ;
...
if (!_bf_addr) throw SE_NOTACTIVE;

drfullpath(fullpath,dbase);
_dbfil=flopen(fullpath,mode);
if (_dbfil)
   {
	_h_db=(char *)memgive(sizeof(DB)+strlen(fullpath));
	_H_DB->h_db_fl=_dbfil;

	if (_lock) _H_DB->db_flg|=F_RDONLY;
...
if (((DB*)db)->db_flg&F_TEMP) drunlink(((DB*)db)->db_fnam);
memtake(db);
_dbfil=0;
}
...
{
dbsafe++; dbclose(db); dbsafe--;
}

int dbreadonly(HDL db)
{
if (((DB*)db)->db_flg&F_RDONLY) return(YES);
return(NO);
}
...
#pragma pack(push,1)
struct XREC  {
   long  totbytes;				// total datasize of this xrec
...
struct XRFCB {
   ...
   HDL			h_db;
   ulong		xrh_rec; /* 'x's needed because of CI-86 */
   ulong		xtotbytes,tot_off;
   ushort		xflags;
...
if ((bkt > 0) && (bkt-- <= x->nused))
   {*p_rhdl=x->bucket[bkt].rh_rec; *psiz=x->bucket[bkt].nbytes;}
}

static void  xtnd(XRFCB *h, void *data, int siz)
{
long	remain=h->xbucket.xnbytes-h->xbucket.off;
int		mov=0;
if (remain>0)
   {
...
if (siz)   /* need to go to next bucket */
   ...
   h->xbucket.off=0;
   if (h->bkt <= h->xnused) xtnd(h,((char*)data)+mov,siz);/* RECURSIVE CALL */
   }
}
...
for (i=0; i<x->nused; i++)
   if (x->bucket[i].nbytes > 0)
      {_recdel(x->bucket[i].rh_rec);x->bucket[i].rh_rec=x->bucket[i].nbytes=0;}
x->totbytes=x->nused=0;
*flg=(short)(pv|DIRTY_PG);	//jjjJ
}


static HDL xrecopen(HDL h_db, RHDL rhdl,char *mode)
{
int		m=TOLOWER(*mode);
XRFCB	*x;
XREC	*xr;
if (!IS_XREC(rhdl) || (m!='a' && m!='r' && m!='w')) return(0);
x=(XRFCB*)memgive(sizeof(XRFCB));
dbactv(x->h_db=h_db);
x->xrh_rec=rhdl;
if (m=='w') xrecrlsbuckets(rhdl);
...
if ((bkt > 0) && (bkt <= x->nused+1))
   ...
   if (bkt > x->nused)
      ...
      if (bkt > x->nbuckets)
			{
			if (x->nbuckets >= MAXBUCKETS) goto OVFLW;
...
if (IS_XREC(x->xrh_rec) && (x->xflags&XM_READ) && x->tot_off<x->xtotbytes)
   {
   dbactv(x->h_db);
   if ((left=x->xtotbytes - x->tot_off) < siz) siz=short(left);
   x->tot_off  += siz;
   xtnd(x, data, siz);
   }
else siz=0;
...
{
dbactv(db);
if (IS_XREC(rhdl)) return(((XREC*)_recadr00(rhdl))->totbytes);
short   siz;
_recadr0(rhdl,&siz);
return(siz);
}
...
else sz=recget(db,rhdl,data,sz);
return(sz);
}
...
{
int	size=zrecsizof(db,rhdl);
if (size)
	{
	void *data=memgive(*sz=size);
	if (zrecget(db,rhdl,data,size)==size)
		{
		*sz=size;
		return(data);
		}
...
{
RHDL	rhdl;
if (sz>MAXRECSIZ) zrecwrite(db,data,sz,rhdl=xrecist(db));
else rhdl=recadd(db,data,(short)sz);
return(rhdl);
}
...
if (IS_XREC(rhdl)!=xrec)		// changed rec to xrec or viceversa
	{recdel(db,rhdl);return(zrecadd(db,data,sz));}
if (xrec)
...
for (i=0;++i<9999;)
	{
	strfmt(n,"TEMP%04d.TMP",i);
	FILEINFO fi;
	if (!drinfo(name,&fi)) break;							// If not exist, use
	if (calnow()-fi.dttm>ONE_DAY) {drunlink(name);break;}	// or if > 1 day old
	}
i=dbsetlock(NO);
if (!dbist(name) || (db=dbopen(name))==NULLHDL)
   throw SE_TMPCREATE;		// Can't create temp database!
dbsetlock(i);
db_set_temp(db);
}
#else
...
char	name[FNAMSIZ];
flnam_env(name,"","TEMP");
char *tn=_tempnam(name,"Transcend_");
int i=dbsetlock(NO);
if (!dbist(tn) || (db=dbopen(tn))==NULLHDL)
   throw SE_TMPCREATE;		// Can't create temp database!
dbsetlock(i);
db_set_temp(db);
//memtake(tn);
...
TMP_BTR::TMP_BTR(int type, int size, PFI_v_v cmppart)
{
btr=btropen(d.db,btrist(d.db,type,size));	// Make + open temp index
if (cmppart) btr_set_cmppart(btr,cmppart);
}

int TMP_BTR::add(RHDL r, void *k)
{
return(bkyadd(btr,r,k));
}

int TMP_BTR::srch(int m,RHDL *r,void *k)
{
return(bkysrch(btr,m,r,k));
}
...
//#define DB_VERIFY YES
#ifdef DB_VERIFY

void	btr_set_formatter(HDL btr,char* (_cdecl *fmt)(short prm, void *key), short parm)
{
H_BTR->formatter=fmt;
H_BTR->formatter_parm=parm;
}

static void  prtkey(HDL ky)
{
short i;
if (_h_btr->formatter)
   prtfmt("%s\r\n",(_h_btr->formatter)(_h_btr->formatter_parm,ky));
else
   {
   for (i=0;i<_btr_ks0;i++) prtfmt("%02X ",ky[i]);
   prtfmt("\r\n");
   }
}

static int  nodeverify
   (RHDL rh_bnode, char *lokey, char *hikey,short list, short *ct, char *inf)
{  // lokey/hikey==prevkey/currkey of parent node entry pointing to this child
short   pos=0,prv;
int	ok=YES;
char  *ky,           // the current key being looked at
      *prevkey;      // previous key looked at (init==prevkey of last node)
BNODE *b=_recadr00(rh_bnode);
prv=dbplockpage(rh_bnode);
prevkey=memadup(lokey,_btr_ks0);
if (list) prtfmt("\r\nNodeType%d Ct%d\r\n",b->node_typ,b->bnkey_ct);
while (pos++ < b->bnkey_ct)
   {
   ky=BNDKEY(b,pos);
   if (b->node_typ!=LEAF)
      {
      if (list) {prtfmt("\r\nLastkey in node=");prtkey(ky);}
      ok=nodeverify(BNDRHDL(b,pos),prevkey,ky,list,ct,inf);
      }
   else (*ct)++;  // quit if reached NOSUCHBUF key
   if (pos==b->bnkey_ct && cmpdocmp(ky,hikey)!=EQ)
      {prtfmt("Err:Last key <> Parent key:");prtkey(hikey);ok=NO;}
   if (cmpdocmp(ky,prevkey)<=0)
      {prtfmt("Err:Key <= Previous key:");prtkey(prevkey);ok=NO;}
   if (*ct>_h_btr->keyct) {prtfmt("Err:Too many keys!");ok=NO;}
   memmove(prevkey,ky,_btr_ks0);
   if (list || !ok) prtkey(ky);
   if (!ok) if (list) ok=YES; else break;
   }
if (list) prtfmt("End of node\r\n");
dbpfreepage(rh_bnode,prv);
memtake(prevkey);
return(ok);
}

short btrverify(HDL h_btr,short list)
{
short   rc,ct=0;
char  *lo,*hi;
btrbind(h_btr);
lo=memgive(_btr_ks0*2);
memset(hi=&lo[_btr_ks0],255,_btr_ks0);		// Create an 'NOSUCHBUF key'
if (_h_btr->keytyp==DT_LONG || _h_btr->keytyp==DT_SHORT)	// If key is 'signed binary',
	{lo[_btr_ks0-1]=0x80;hi[_btr_ks0-1]=0x7F;}		// lowest/highest possible
if (list!=1) list=0;
rc=nodeverify(_h_btr->rh_root,lo,hi,list,&ct,hi);
memtake(lo);
if (list) prtfmt("Total keys %d\r\n",ct);
return(rc);
}
#endif	// of DEBG

#ifdef docum
Functions SETTING/testing actv_prcd
BNDREPOS				(if n/f set NO, else no change)
TRAVSBRANCH			(set YES if n/f and no higher keys, else NO)
_BKYDEL				(set NO)
ADD_BKEY_TO_NODE	(if node is a leaf set YES, else no change)
BKYADD				(set YES)
BKYSETPOS			(set NO)
_Bkysetnxthdl		(tests, sets NO if recurse, else YES)
_Bkysetprvhdl		(always tests and sets)
Bkypos				(temporarily sets YES if LST_DUP)
Bkyscn				(temporarily sets NO if BK_CUR)
Bkysrch				(always sets NO if key is found)

Functions SETTING/testing actv_exists
TRAVSBRANCH			(set YES if found, else NO)
_BKYDEL				(set NO)
ADD_BKEY_TO_NODE	(if node is a leaf set YES, else no change)
BKYSETPOS			(set NO)
_Bkysetnxthdl		(no check. If no recurse - NO if n/f & no higher, else YES)
_Bkysetprvhdl		(always checks and sets)
Bkyscn				(check only if BK_CUR)

......			is called by...
bndrepos
	_Bkydel
		BKYDEL
	_Bkysetnxthdl (recurses)
		Bkynxtkey
	_Bkysetprvhdl (recurses)
		Bkyprvkey
Travsbranch
	Bkyfnd			(this one never cares about prcd/actv flags)
		BKYADD
		BKYDEL
		BKYPOS
		BKYSRCH
